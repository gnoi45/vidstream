...
def main():
  r = requests.post('192.168.43.94', json={'_key': 1234})
  ...
  while True:
    ...
    if image_data_content != PREV_IMG:
      files = {}
      filename = str(round(time.time()*1000))+'_'+1234
      files[filename] = ('img.png', image_data_content, 'multipart/form-data')

      try:
        r = requests.post('192.168.43.94'+'/capture_post', files=files)
    ...
    # events
    try:
      r = requests.post('192.168.43.94'+'/events_get', json={'_key': 1234})
      data = r.json()
